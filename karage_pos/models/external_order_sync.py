...
class ExternalOrderSync(models.Model):
    ...
    def _get_or_create_pos_session(self):
        ...
        if self.auto_create_session:
            _logger.info(f'Creating new POS session for config {self.pos_config_id.name}')
            try:
                # Create session with system user
                system_user = self.env.ref('base.user_admin')
                session = self.env['pos.session'].sudo().with_user(system_user).create({
                    'config_id': self.pos_config_id.id,
            ...
            raise UserError(_('No open POS session found and auto-create is disabled. Please open a POS session manually.'))
    ...
    def _fetch_orders_from_external(self):
        ...
        try:
            ...
            raise UserError(_('Failed to fetch orders from external system: %s') % str(e))
        except Exception as e:
    ...
    def _process_external_order(self, order_data):
        ...
        try:
            ...
            for order_item in order_data.get('OrderItems', []):
                ...
                if not product:
                    _logger.warning(f'Product not found: ItemName="{item_name}", ItemID={item_id}')
                    continue
            ...
            if not order_lines:
                _logger.warning(f'No valid order lines for order {order_data.get("OrderID")}')
                return False
            ...
            if not payment_lines:
                _logger.warning(f'No valid payment lines for order {order_data.get("OrderID")}')
                return False
            ...
            _logger.info(f'Created POS order {pos_order.name} from external order {order_data.get("OrderID")}')
            return pos_order
            
        except Exception as e:
            _logger.error(f'Error processing external order {order_data.get("OrderID")}: {str(e)}', exc_info=True)
            return False
