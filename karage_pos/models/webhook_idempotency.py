...
class WebhookIdempotency(models.Model):
    ...
    order_id = fields.Char(
        string="Order ID", help="External Order ID from the webhook request"
    )
    pos_order_id = fields.Many2one(
        "pos.order", string="POS Order", help="Created POS order from this webhook"
    )
    ...
    response_data = fields.Text(
        string="Response Data", help="JSON response data from the webhook processing"
    )
    error_message = fields.Text(
        string="Error Message", help="Error message if processing failed"
    )
    processed_at = fields.Datetime(
        string="Processed At", help="Timestamp when the request was processed"
    )
    ...
    def create_idempotency_record(
        self, idempotency_key, order_id=None, status="pending"
    ):
        """
