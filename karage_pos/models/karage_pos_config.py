...
class KaragePosConfig(models.Model):
    ...
    api_key = fields.Char(string='API Key', required=True, help='API key to authenticate webhooks sent to Odoo. This key should be included in the X-API-KEY header when sending webhooks.')
    active = fields.Boolean(string='Active', default=True)
    odoo_url = fields.Char(string='Odoo URL', help='Base URL of the Odoo instance (e.g., https://odoo.example.com)')
    webhook_endpoint = fields.Char(string='Webhook Endpoint', default='/api/v1/webhook/pos-order', help='Endpoint path for webhook requests')
