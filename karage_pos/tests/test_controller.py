...
class TestWebhookController(HttpCase, KaragePosTestCommon):
    ...
    def test_webhook_with_idempotency_key(self):
        ...
        # First request
        response1 = self._make_webhook_request(self.sample_webhook_data, headers=headers)
        self.assertEqual(response1.status_code, 200)
        result1 = json.loads(response1.content)
        order_id_1 = result1['data']['id']
        
        # Second request with same idempotency key
        response2 = self._make_webhook_request(self.sample_webhook_data, headers=headers)
